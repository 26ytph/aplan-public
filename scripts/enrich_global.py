...
async def main():
    ...
    with open("scripts/data/enriched_global_gourmet.json", "r", encoding="utf-8") as f:
        pois = json.load(f)
        
    print(f"📥 準備匯入 {len(pois)} 筆全球化美食菁英 (Global Gourmet)...")
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        for poi in pois:
            ...
            try:
                ...
                print(f"✅ 成功匯入並向量化: {poi['name']} (Tier {poi.get('tier', 3)})")
                inserted_count += 1
